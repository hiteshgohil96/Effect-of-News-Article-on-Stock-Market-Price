...
Source code to get Stocks data from Alpha Vantage documentation
Source code to get Stocks data from Google News API documentation
Source code to analyze text data was adapted from analyticsvidhya
Source code to do text processsing was implemented from ahmedbesbes
Source code to perform sentiment alalysis using VaderSentiment was adapted from medium
Source code to to construct a forecasting model was implemented from github/LastAncientOne
Source code to combine stocks and News according to their date was implemented from shanelynn
Source code to combine stocks and News according to their date was implemented fromstackoverflow
Source code to plot candle stick was implemented from stackoverflow
Source code to plot second y axis was implemented from stackoverflow
Source code to plot time series was implemented from plot.ly
Additional reference to do Sentiment Analysis towardsdatascience
Additional reference to do text processing medium
Requirements
In this section, provide detailed instructions for installing any necessary pre-requisites. This could include: Python packages/libraries to be installed:
...
py.iplot(fig)
Image of Plot Plot only close value
...
Ensure that you have registered for the Google News API key. (You may reference the instructions for doing this.)
Register for the Alpha Vantage API key. 4.Open "projectResults.ipynb" file in jupyter Notebook and run the commands to see the analysis and the animated plots.
...
This plot shows how many relevant articles are shown on each day gathered from Google news API. Image of Plot
...
We predicted the stocks using two different models, predicting stock without sentiment analysis and with sentiment analysis. prediction1 = 190.59768192 prediction2 = 190.08 percentage change(prediction1,prediction2) in prediction =-0.27160977

Future work:
1.To make the sentiment analysis more accurate we need to have more data that is taking from other data sources. 2.Implement other methods to do sentiment analysis such as Bag of Words etc. 3.Visualize poitive words using a word cloud. 4.Impact of different sources can be analysed on stock closing price, which sources has maximum impact on stock by their news article. 5.We can see in the results that the model overfits data because of the less data points.
